...
theorem distance_value (t : Fin 4 → ℝ) (k : Fin 10) :
  ...
  · change _ = 4 * ((0)^2 +
    ...
    norm_num [distances, FixedPointSound.Expr.val] <;> ring
  · change _ = 4 * ((0)^2 +
        ((2 : ℝ) * (t 0) + (-2 : ℝ) * (t 3))^2) / ((1 + (t 0)^2) * (1 + (t 3)^2))
    norm_num [distances, FixedPointSound.Expr.val] <;> ring
  · change _ = 4 * ((0)^2 +
        ((2 : ℝ) * (t 1) + (-2 : ℝ) * (t 2))^2) / ((1 + (t 1)^2) * (1 + (t 2)^2))
    norm_num [distances, FixedPointSound.Expr.val] <;> ring
  · change _ = 4 * ((0)^2 +
        ((2 : ℝ) * (t 1) + (-2 : ℝ) * (t 3))^2) / ((1 + (t 1)^2) * (1 + (t 3)^2))
    norm_num [distances, FixedPointSound.Expr.val] <;> ring
  · change _ = 4 * ((0)^2 +
...
theorem distances_values (t : Fin 4 → ℝ) :
    distances.map (fun e => e.val t) = List.ofFn (fun k : Fin 10 => certDistance 0 k t) := by
...
theorem objective_eq (t : Fin 4 → ℝ) : objective t = certObjective 0 t := by
  simp only [objective, distances_values, List.prod_ofFn, certObjective]

theorem admissible_of_geometry (t : Fin 4 → ℝ) (ha : admissible (treePoints 0 t)) :
    FixedPointSound.Admissible distances t := by
  intro e he
  have hm : e.val t ∈ List.ofFn (fun k : Fin 10 => certDistance 0 k t) := by
    rw [← distances_values]
    exact List.mem_map.mpr ⟨e,he,rfl⟩
  obtain ⟨k,hk⟩ := List.mem_ofFn.mp hm
  rw [← hk]
  exact ⟨certDistance_nonneg 0 k t, certDistance_le_four 0 k t ha⟩
